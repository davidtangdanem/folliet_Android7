...
public class dbKD543LinInventaire extends dbKD {
	...
	public boolean  bLigneQteExiste(String numcde,String codearticle)
	{
		boolean bres=false;
		...
		try
		{
			...
			String query="select * from "+table+" where "+
					...
					" and "+
						Global.dbKDLinInv.FIELD_LIGNEINV_QTE+"<>''"+
						" and "+
						Global.dbKDLinInv.FIELD_LIGNEINV_QTE+"<>'0'"+
						" and "+
						 this.FIELD_LIGNEINV_NUMDOC+"='"+numcde+  "'"+
						 	" and "+
						 this.FIELD_LIGNEINV_PROCODE+"='"+codearticle+  "'";
	...
	public List<structPassePlat> loadSupZero()
	{
...
/*			String query="UPDATE "+TABLENAME_TEMP2+
	...
	public void decrementeStock(String numcde,Context act)
	{
		...
		for (int i=0;i<arts.size();i++)
		{
			...
			Global.dbKDLinInv.load(pp, arts.get(i).PROCODE, new StringBuffer(),
			...
			Global.dbKDLinTransfert.load(lin, arts.get(i).PROCODE, new StringBuffer(),
					Preferences.getValue(act, Espresso.DEPOT,"0"),
					"1",
					"1",
					Preferences.getValue(act, Espresso.LOGIN,"0"),
			...
			*/
			...
			int qteStock=Fonctions.convertToInt( pp.FIELD_LIGNEINV_QTETHEO)-
		...
		//	lin.FIELD_LIGNEMVTS_QTETHEO=qteStock+"";
			...
			Global.dbKDLinInv.save(pp, arts.get(i).PROCODE, new StringBuffer());
			//Global.dbKDLinTransfert.save(lin, arts.get(i).PROCODE, new StringBuffer());
