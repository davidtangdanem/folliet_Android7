...
/**
 * Activity permettant de gérer les remises en banque
...
public class RemiseEnBanque extends BaseActivity implements OnClickListener{
	...
	ListView lvCheques;
	TextView tvTotalSelectionCheque, tvTotalEspece, tvTotalADeposer, tvTotalChequeJour, tvTotalChequePortefeuille;

	ArrayList<Encaissement> mEncaissementsCheque;
	ArrayList<Encaissement> mEncaissementsChequePortefeuille;
	...
	String m_MontantChequeCHP="";
	String m_MontantChequeALL="";
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		//Récupération des encaissements enregistrés de type chèque
		mEncaissementsChequePortefeuille = Encaissement.getEncaissementsFromTypePaiement(Encaissement.TYPE_CHEQUEPORTEFEUILLE);
		
		mEncaissementsCheque.addAll(mEncaissementsChequePortefeuille);
		...
		mEncaissementsCheque.addAll(mEncaissementsEspece);
	...
	private void initTotalAdeposer() {
		...
		float total = 0;
		total=Fonctions.GetStringToFloatDanem(m_MontantEspece)+Fonctions.GetStringToFloatDanem(m_MontantCheque)+Fonctions.GetStringToFloatDanem(m_MontantChequeCHP);
		float totalAllCheque = 0;
		totalAllCheque=Fonctions.GetStringToFloatDanem(m_MontantCheque)+Fonctions.GetStringToFloatDanem(m_MontantChequeCHP);
		...
		m_MontantChequeALL=Fonctions.GetFloatToStringFormatDanem(totalAllCheque, "0.00");
	...
	private void initGUI() {
		...
		tvTotalChequeJour = (TextView) findViewById(R.id.tvTotalChequeJour);
		tvTotalChequePortefeuille = (TextView) findViewById(R.id.tvTotalChequePortefeuille);
	...
	private void initChequeAdapter() {
		...
		mAdapter = new ChequeAdapter(this, R.layout.item_list_cheques, mCheques, new OnChequeCheck() {
			...
			public void onCheck(float totalChequeJour, float totalChequePortefeuille, float totalCheque, float totalEspece) {			
				displayTotalCheque(totalCheque);
				displayTotalEspece(totalEspece);
				displayTotalChequeJour(totalChequeJour);
				displayTotalChequePortefeuille(totalChequePortefeuille);
				initTotalAdeposer();
			}
	...
	private void displayTotalChequeJour(float total) {
		tvTotalChequeJour.setText(Fonctions.GetFloatToStringFormatDanem(total, "0.00"));
		m_MontantCheque=Fonctions.GetFloatToStringFormatDanem(total, "0.00");
	}
	
	private void displayTotalChequePortefeuille(float total) {
		tvTotalChequePortefeuille.setText(Fonctions.GetFloatToStringFormatDanem(total, "0.00"));
		m_MontantChequeCHP=Fonctions.GetFloatToStringFormatDanem(total, "0.00");
	}

	private void displayTotalEspece(float total) {
		tvTotalEspece.setText(Fonctions.GetFloatToStringFormatDanem(total, "0.00"));
		m_MontantEspece=Fonctions.GetFloatToStringFormatDanem(total, "0.00");
	}
	...
	void save ()
	{
		...
		banque.MNT_CHEQUE_CHP=m_MontantChequeCHP;
		banque.MNT_CHEQUE_ALL=m_MontantChequeALL;
		...
		if(mCheques != null && mCheques.size() > 0){
			for(Cheque cheque : mCheques){
				if(cheque.isChecked()) 
				{
					...
					stTypeEncaissement=cheque.getTypeEncaissement();
					...
					banquelin.TYPE_ENCAISSEMENT=stTypeEncaissement;
