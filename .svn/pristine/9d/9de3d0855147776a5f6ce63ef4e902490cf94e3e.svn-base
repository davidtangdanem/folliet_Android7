...
public class dbKD982RetourBanqueLin extends dbKD{
	...
	//Type
	public final String FIELD_RLBQ_TYPE_ENCAISSEMENT = fld_kd_dat_data06;
	...
	static public class structRetourBanqueLin {
		...
		public String TYPE_ENCAISSEMENT;
	...
	public boolean insertRetourBanqueLin(structRetourBanqueLin retourbanque){
		...
		try{
			...
			query = "INSERT INTO "
					...
					+ ","
					+ this.FIELD_RLBQ_TYPE_ENCAISSEMENT
					...
					+ ","
					+ "'"+MyDB.controlFld(retourbanque.TYPE_ENCAISSEMENT)+"'"
					+ ")";

			db.conn.execSQL(query);
			Global.dbLog.Insert("982RETOURBANQUE", "INSERT", "", "",  query, "", "");
	...
	public void updateremiseChequeportefeuille(){
		...
		try {
			String query="SELECT * FROM "+
					...
					" and "+
					this.FIELD_RLBQ_TYPE_ENCAISSEMENT+"='CH' and "
					+ this.FIELD_RLBQ_DATE + "!='"+Fonctions.getYYYYMMDD()+"'";
			...
			if (cur!=null )
			{
				while(cur.moveToNext())
				{
					 query = "update " + TABLENAME +
								" set "+this.FIELD_RLBQ_TYPE_ENCAISSEMENT+"='CHP'"+
								" where "
		...
		ArrayList<structRetourBanqueLin> lines = new ArrayList<dbKD982RetourBanqueLin.structRetourBanqueLin>();
			...
			if (cur!=null)
			{
				...
				while (cur.moveToNext())
				{
					...
					line.TYPE_ENCAISSEMENT = this.giveFld(cur,this.FIELD_RLBQ_TYPE_ENCAISSEMENT);
					lines.add(line);
				}
				cur.close();
			}
