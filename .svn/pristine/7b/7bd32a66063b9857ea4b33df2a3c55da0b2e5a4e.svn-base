...
//			Debug.StackTrace(e);
	...
	static String getServiceResponse(SoapSerializationEnvelope envelope) {
		try {
			...
			//SoapPrimitive resultString = (SoapPrimitive) envelope.getResponse();
			//Object resultString = (Object) envelope.getResponse();

			Log.e("resultString",""+resultString.toString());

			if (resultString.getPropertyCount() == 0)
			{
				result = "";
			} else {
			...
			Log.e("ExceptionGet","ServiceResponse =>"+ex);
...
*/
		...
		public static Result WSQueryZippedExBorne(String user, String pwd,
			...
			try {
				...
				request.addProperty("stPwd", pwd);
				request.addProperty("stScenar", scenario);
				request.addProperty("stFilter", filtre);// AND
				request.addProperty("start", start);
				request.addProperty("count", count);

				Log.e("scenario",""+scenario);
				//Log.e("start","start=> "+start);
				//Log.e("count","count=> "+start);
