...
public class Z420ModelPretMachine {

	Context context;
	
	public Z420ModelPretMachine(Context c)
	{
		context=c;
	}
 
	public String getPretEx( String codecli  )
	{	 
		String completeLine="";
		
		dbKD452ComptageMachineclient ret=new dbKD452ComptageMachineclient(Global.dbParam.getDB());
		
		ArrayList<Bundle> sret=ret.getListeMateriel(codecli );
		int nbrline=sret.size();
		if (nbrline==0) return "";
		
		String numdoc=sret.get(0).getString(Global.dbKDComptageMachineClient.FIELD_CMC_NUMSOUCHE);
				
		int taillepage=1000+50*(nbrline*2);//on double les lignes a cause des commentaires
		...
		line+="^CF0,25\n" + 
				...
				"^FO210,90^FDMISE A DISPOSITION DE MACHINE^FS\n" + 
				"^FO25,140^FDDate : IMPR_DATEIMPR^FS \n" + 
				"^FO450,140^FDImprimé le : IMPR_DATEHEURE^FS \n" + 
				" ^FO25,170^FDNum prêt : IMPR_NUMDOC^FS\n" + 
				"\n" + 
	...
	int imprimeLine(int currentY,String codecli, StringBuffer sb)
	{
		...
		dbKD452ComptageMachineclient lin=new dbKD452ComptageMachineclient(Global.dbParam.getDB());
		ArrayList<Bundle>  lines=lin.getListeMateriel(codecli);
		String totaline="";
		String comment="";
		String totalComment="";
		...
		{
			String detail=line;
			detail=detail.replace("IMPR_CODEART", lines.get(i).getString(Global.dbKDComptageMachineClient.FIELD_CMC_CODEART));
			detail=detail.replace("IMPR_DESIGNATION",Fonctions.Left(lines.get(i).getString(Global.dbKDComptageMachineClient.FIELD_CMC_LBL_ART),19));
			detail=detail.replace("IMPR_NUMSZ",lines.get(i).getString(Global.dbKDComptageMachineClient.FIELD_CMC_NUMSERIESZ));
			detail=detail.replace("IMPR_NUMSERIE", lines.get(i).getString(Global.dbKDComptageMachineClient.FIELD_CMC_NUMSERIEFABRICANT));
 
			comment=lines.get(i).getString(Global.dbKDComptageMachineClient.FIELD_CMC_COMMENTAIRE);
			if (comment.equals("")==false);
				totalComment+=(lines.get(i).getString(Global.dbKDComptageMachineClient.FIELD_CMC_NUMSERIESZ)+" : "+comment+"|");
