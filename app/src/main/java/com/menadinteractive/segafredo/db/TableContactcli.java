...
public class TableContactcli extends DBMain{
	...
	static public String TABLENAME="CONTACTCLI2";
...
//	static public final String FLAG_UPDATE="1";
	...
	//Serveur
	public static final String FIELD_SRV_CODECONTACT 		= "CON_CODECONTACT";
	...
	public static final String FIELD_SRV_CIVIL			= "CON_CIVIL";//
	...
	public static final String TABLE_CREATE="CREATE TABLE ["+TABLENAME+"] ("+
			...
			" ["+FIELD_FLAG		+"] [nvarchar](10) NULL"	 + COMMA +
			" ["+FIELD_CIVIL		+"] [nvarchar](50) NULL"	 +
...
ee=ex.getLocalizedMessage();
	...
	void fillStruct(Cursor cur,structContactcli cli)
	{
		...
		cli.CIVIL=cur.getString(cur.getColumnIndex(Global.dbContactcli.FIELD_CIVIL));	
	...
	public ArrayList<Bundle> getContactcliFilters(String codeclient,  String filter)
	{
		try
		{
			...
			while (cur.moveToNext())
			{
				...
				cli.putString(Global.dbContactcli.FIELD_NOM, Global.dbParam.getLblAllSoc(Global.dbParam.PARAM_TITRECONTACT,cur.getString(cur.getColumnIndex(Global.dbContactcli.FIELD_CIVIL))) +" "+cur.getString(cur.getColumnIndex(Global.dbContactcli.FIELD_NOM)));				
				...
				cli.putString(Global.dbContactcli.FIELD_CIVIL, Global.dbParam.getLblAllSoc(Global.dbParam.PARAM_TITRECONTACT,cur.getString(cur.getColumnIndex(Global.dbContactcli.FIELD_CIVIL))));
	...
	public boolean EmailContactcliExiste(String codeclient,   String codecontact)
	{
		boolean bres=false;
		String email="";
		try
		{
			...
			while (cur.moveToNext())
			{
				email=Fonctions.GetStringDanem(cur.getString(cur.getColumnIndex(Global.dbContactcli.FIELD_EMAIL))).trim();
				if(!email.equals(""))
				 bres=true;
	...
	public structContactcli load(Cursor cursor){
		...
		if(cursor != null){
			...
			client.CIVIL = giveFld(cursor, FIELD_CIVIL);
