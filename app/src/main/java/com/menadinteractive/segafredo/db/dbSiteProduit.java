...
public class dbSiteProduit extends DBMain {
	public static String TABLENAME = "SITE_PRODUITV4";
	...
	public final String FIELD_PRO_EMPLAC = "EMPLAC";// NO UTILISER
	...
	public final String FIELD_PRO_QTE_PAL = "QTE_PAL"; // QTE_PAL
	public final String FIELD_PRO_DAT_MAX_REAPRO = "PRO_DAT_MAX_REAPRO"; // QTE_PAL
	...
	public static final String TABLE_CREATE = "CREATE TABLE IF NOT EXISTS [SITE_PRODUITV4]("
			+ ID
			...
			+ "		[QTE_PAL] [nvarchar](30) NULL,"
			+ "		[PRO_DAT_MAX_REAPRO] [nvarchar](30) NULL,"
	...
	static public class structArt {
		...
		public String QTE_PAL;
		
		public String INV;
		public String PRO_DAT_MAX_REAPRO;
	...
	public ArrayList<structArt> getProduitsInventaire(boolean dejavu,boolean pasvu,String codefam,String filtre,boolean m_ballarticle) {
		ArrayList<structArt> arts = new ArrayList<dbSiteProduit.structArt>();

		String innerjoinallarticle=" inner join ";
		if(m_ballarticle==true)
		{
			innerjoinallarticle=" left join ";
		}
		try {
			...
			query = "select pro.*,"
					...
					+ innerjoinallarticle+ dbKD545StockTheoSrv.TABLENAME
					+ " as lin " + " on "
					...
					+" inv."+dbKD543LinInventaire.FIELD_LIGNEINV_PROCODE+"=pro."+FIELD_PRO_CODART
			...
			while (cur != null && cur.moveToNext()) {
				...
				if(Fonctions.GetStringDanem(giveFld(cur, dbKD545StockTheoSrv.FIELD_LIGNEINV_PROCODE)).equals(""))
				art.INV="0";
				else
					art.INV="1";
	...
	public ArrayList<structArt> getProduitsEchangeStock(boolean dejavu,boolean pasvu,String codefam, String filtre,boolean m_ballarticle) {
		ArrayList<structArt> arts = new ArrayList<dbSiteProduit.structArt>();

		try {
			String LeftJoin=" left join ";
			if(codefam.equals("Déjà échangé"))
			{
				 LeftJoin=" inner join ";
			}
			...
			query = "select pro.*,"
					...
					+  " lin."+dbKD545StockTheoSrv.fld_kd_dat_type + "="
					+  dbKD545StockTheoSrv.KD_TYPE 
					+LeftJoin+" "+dbKD546EchangeStock.TABLENAME +" as inv on "
					+" inv."+dbKD546EchangeStock.FIELD_PROCODE+"="+this.FIELD_PRO_CODART
			...
			{
				query=query.replace("FILTREFAM", "");
				query=query.replace("FILTREDEJAINV", " and  (inv."+dbKD546EchangeStock.FIELD_QTE+"  is null or inv."+dbKD546EchangeStock.FIELD_QTE+" ='' ) " );
			}
			else if (dejavu)
			{
				query=query.replace("FILTREFAM", "");
				query=query.replace("FILTREDEJAINV", " and  (inv."+dbKD546EchangeStock.FIELD_QTE+"  is not null and inv."+dbKD546EchangeStock.FIELD_QTE+" <>'' ) " );
			}
			else if (codefam.equals("")==false)
			{
				query=query.replace("FILTREDEJAINV", "");
			...
			while (cur != null && cur.moveToNext()) {
				...
				art=fillStructArt(cur,art);
				//art.QTEINV = giveFld(cur, dbKD543LinInventaire.FIELD_LIGNEINV_QTE);
				art.QTEINV = giveFld(cur, dbKD546EchangeStock.FIELD_QTE);
	...
	public ArrayList<structArt> getProduitsWithHisto(String soc_code, String codecli, String codefam,
			String filtre,String codetarif,String typedoc,boolean m_tech) {
		ArrayList<structArt> arts = new ArrayList<dbSiteProduit.structArt>();

		Log.e("debut","getProduitsWithHisto");
		try {
			...
			String inner_JoinCodeTarif="";
			if(!codetarif.equals("") && (!codetarif.equals("99") && !codetarif.equals("GEN")) )
			{
				inner_JoinCodeTarif=" inner join "+TableTarif.TABLENAME+" trf on art."+FIELD_PRO_CODART+"=trf."+TableTarif.FIELD_COD_PRO+" and trf."+TableTarif.FIELD_COD_CLI+"='"+codetarif+"'";
			}
			String wheredatereappro="";
			if(typedoc.equals(TableSouches.TYPEDOC_COMMANDE))
			{
				wheredatereappro="and ( art."+FIELD_PRO_DAT_MAX_REAPRO+">"+Fonctions.getYYYYMMDD()+" OR art."+FIELD_PRO_DAT_MAX_REAPRO+" IS NULL "+" OR art."+FIELD_PRO_DAT_MAX_REAPRO+"='' ) ";
			}
			...
			query = "select "+
					...
					" or hdr."+dbKD729HistoDocuments.FIELD_TYPEDOC+"='"+TableSouches.TYPEDOC_PROFORMA+"')"+
					inner_JoinCodeTarif
					+" where "
					+ " art."+FIELD_PRO_SOC_CODE + "='" + soc_code + "'" + queryadd
					+ " and " + FIELD_PRO_ACTIF_CDE + "<>'N'" +wheredatereappro+
					" group by "+
					"art."+FIELD_PRO_CODART+","+
					"art."+FIELD_PRO_PCB+","+
					"art."+FIELD_PRO_CODETVA+","+
			...
			if (queryadd.equals("") && m_tech==true )
			{
			...
			Log.e("query"," getproduitswithhisto " + query);
		...
		Log.e("fin","getProduitsWithHisto");
	...
	structArt fillStructArt(Cursor cur,structArt art)
	{
		...
		art.QTE_PAL = giveFld(cur, this.FIELD_PRO_QTE_PAL);
		art.PRO_DAT_MAX_REAPRO = giveFld(cur, this.FIELD_PRO_DAT_MAX_REAPRO);
	...
	public String getCodearticle_EAN(String code)
	{
		String libelle="";
		...
		try
		{
			...
			if (cur.moveToNext())
			{
				libelle=giveFld(cur,FIELD_PRO_CODART          );
	...
	public String getFam(String code)
	{
		String libelle="";
		...
		try
		{
			...
			if (cur.moveToNext())
			{
				libelle=giveFld(cur,FIELD_PRO_CODFAM          );
	...
	public static class Comparators {
		...
		public static Comparator<structArt> DATE = new Comparator<structArt>() {
			@Override
			public int compare(structArt o1, structArt o2) {
				if (o1 == null) {
					return (o2 == null) ? 0 : -1;
				}
				if (o2 == null) {
				...
				if (o1.MAXDATE == null) {
					return (o2.MAXDATE == null) ? 0 : -1;
				}
				if (o2.MAXDATE == null) {
					return 1;
				}
				if (o1.MAXDATE.equals("")) {
					return (o2.MAXDATE.equals("")) ? 0 : -1;
				}
				if (o2.MAXDATE.equals("")) {
					return 1;
				}
				return Fonctions.getYYYYMMDD(o2.MAXDATE).compareTo(Fonctions.getYYYYMMDD(o1.MAXDATE));
			}
		};

	}
	public String  getProduitsWithHisto_chargement(String codearticle, String codecli) {
		...
		try {
            ...
            query = "select lin."+
                    dbKD731HistoDocumentsLignes.FIELD_CODEART+","+
                    "max(hdr."+dbKD729HistoDocuments.FIELD_DATEDOC+") maxdate "+

                    " FROM "+dbKD731HistoDocumentsLignes.TABLENAME_HISTO+" lin "+
                    " inner join "+dbKD729HistoDocuments.TABLENAME_HISTO+" hdr "+
                    " on hdr."+dbKD729HistoDocuments.fld_kd_dat_type+"="+dbKD729HistoDocuments.KD_TYPE+
                    " and lin."+dbKD731HistoDocumentsLignes.FIELD_NUMDOC+"=hdr."+dbKD729HistoDocuments.FIELD_NUMDOC+
                    " and (hdr."+dbKD729HistoDocuments.FIELD_TYPEDOC+"='"+TableSouches.TYPEDOC_FACTURE+"'"+
                    " or hdr."+dbKD729HistoDocuments.FIELD_TYPEDOC+"='"+TableSouches.TYPEDOC_PROFORMA+"')"
                    ...
                    +" where "
                    + " lin."+dbKD731HistoDocumentsLignes.FIELD_CODEART+ "='" + codearticle + "'"
                    + " and lin." + dbKD731HistoDocumentsLignes.fld_kd_dat_type+"="+dbKD731HistoDocumentsLignes.KD_TYPE
					+ " order by  hdr." + dbKD729HistoDocuments.fld_kd_dat_idx04+" desc" ;
			...
			Cursor cur = db.conn.rawQuery(query, null);
			while (cur != null && cur.moveToNext()) {
				MaxDate =Fonctions.YYYY_MM_DD_to_dd_mm_yyyy( giveFld(cur, "maxdate"));
				break;
			}
			cur.close();
