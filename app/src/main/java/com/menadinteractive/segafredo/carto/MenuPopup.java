...
public class MenuPopup implements OnClickListener, OnItemClickListener{
	...
	TextView tv_fermetures, tvEvenement;
	...
	public MenuPopup(Context context, Handler handler, structClient structClient,structSoc soc,boolean m_btech,String m_streplogin,MyDB db){
	...
	public void updatePopup(structClient structClient, boolean display){
		...
		{
			m_bvisite=true;
			Global.dbKDVisite.load_evt_id(this.visite, structClient.CODE, jourPassage,Fonctions.GetStringDanem(structClient.CLI_TO_VISIT_STRUCT.FIELD_EVT_ID), new StringBuffer());
	
		}
		else
		Global.dbKDVisite.load_evt_id(this.visite, structClient.CODE, jourPassage,"", new StringBuffer());
		...
		if(this.m_btech==true)
    	{
			...
			{
				dbTournee db=new dbTournee(this.DB);
	    		m_stnuminterne=db.CodeNum_interne(structClient.CODE);
	    		TableHistoInter hi=new TableHistoInter(this.DB);
	    		m_stnumfab=hi.LoadInfo_numfab(structClient.CODE,m_stnuminterne);
	    		m_typeinter=hi.LoadType_inter(structClient.CODE,m_stnuminterne);
			}
		...
		if(structClient.CLI_TO_VISIT_STRUCT != null 
				&& structClient.CLI_TO_VISIT_STRUCT.FIELD_LBL != null 
				&& !structClient.CLI_TO_VISIT_STRUCT.FIELD_LBL.equals("")){
			tvEvenement.setVisibility(View.VISIBLE);
			tvEvenement.setText(structClient.CLI_TO_VISIT_STRUCT.FIELD_LBL.trim() );
		...
		/*if(structClient.FREETEXT != null && !structClient.FREETEXT.equals("")){
			tvEvenement.setVisibility(View.VISIBLE);
			tvEvenement.setText(structClient.FREETEXT.trim());
		}else{
			tvEvenement.setVisibility(View.GONE);
		}*/
	...
	private void initModels(){
		...
		//items.add(new MenuPopupItem(R.string.commande_Historique, R.drawable.basic1_104_database_download));
		items.add(new MenuPopupItem(R.string.rapport, R.drawable.rapport_de_visites));
		...
		items.add(new MenuPopupItem(R.string.menu_client, R.drawable.basic2_105_user));
		if(m_btech==true)
		{
			items.add(new MenuPopupItem(R.string.menu_materiel, R.drawable.basic2_295_dashboard_materiel));
			items.add(new MenuPopupItem(R.string.menu_sav, R.drawable.basic2_292_tools_settings));
	...
	static public void launchRapport(Context c,String codecli,String code_evt,String evt_id,String numInterne) {
		...
		//recherche, si dispo, du premier rdv pour le client courant
		dbKD106AgendaCorrespondance db106= new dbKD106AgendaCorrespondance (Global.dbParam.getDB() ) ;
		dbKD106AgendaCorrespondance.passePlat rr = db106.getFistRdvForCli(codecli) ;
		if (  rr != null )
		{
			b.putString("datedeb", rr.FIELD_DTDEBUT);
			b.putString("datefin", rr.FIELD_DTFIN);
		}
	...
	void launchFicheMaterielClient()
	{
...
//			context.startActivity(messIntent);
			Fonctions._showAlert(context.getString(R.string.num_tel)+" "+structClient.TEL1, context);
		}
		else if(l.getLabelId() == R.string.menu_client){
...
//			ArrayList<structStock> stocks = Global.dbStock.getAllAsList("400006B3MCOT");
			...
			//launchMedia();
		}else if(l.getLabelId() == R.string.rapport){
			//Dialog pour laisser un commentaire
			//ShowDialogComment();
		//	launchQuest(context, m_codeCli, "", "");
			launchRapport(context,m_codeCli,m_code_evt,m_evt_id,m_stnuminterne);
			
		}
		else if(l.getLabelId() == R.string.menu_materiel){
		
	
			if(!Fonctions.GetStringDanem(structClient.CODE).equals("ST"+this.m_streplogin)){
			
				launchFicheMaterielClient();
			}
			else
				Fonctions.FurtiveMessageBox(context, "Pas de matériel client");
			
		}
		else if(l.getLabelId() == R.string.menu_sav){
			
			if(!Fonctions.GetStringDanem(structClient.CODE).equals("ST"+this.m_streplogin)){
			
				launchFicheHistoInter();
			}
			else
				Fonctions.FurtiveMessageBox(context, "Pas de SAV client");
	...
	void ShowDialogComment(){
		...
		.setMessage(context.getString(R.string.client_vu_commentaire))
		.setView(et)
		.setPositiveButton(R.string.quest_save, null) //Set to null. We override the onclick
		.setNegativeButton(R.string.quest_cancel, null)
		.create();
				...
				b.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View view) {
						if(et.getText() != null && !et.getText().toString().equals("")){
							CartoMapActivity.updateVisite(et.getText().toString(), structClient, false,context);					
							updateUIFull();
							closePopup();
							d.dismiss();
						}else{
							Toast.makeText(context, context.getString(R.string.commentaire_obligatoire), Toast.LENGTH_SHORT).show();
						}
